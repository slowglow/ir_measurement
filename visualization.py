...
LAST_RECORDING_PATH = "recorded/lastRecording/RIR.npy"
LAST_RECORDING_STIMULUS_PATH = "recorded/lastRecording/sigtest.wav"
FS = 44100
...
def main():
    ...
    rir = np.load(LAST_RECORDING_PATH)
    maxval = np.max(rir)
    minval = np.min(rir)
    ...
    sigtest, ff = sf.read(LAST_RECORDING_STIMULUS_PATH)
    ...
    ff, tt, spectest = spectrogram(
        sigtest, nperseg=nperseg, nfft=sweepnfft, noverlap=int(0.5*nperseg), scaling="spectrum")
    spectest = spectest[1::, :]

    for idx in range(sigrec.shape[1]):

        ff, tt, specrec = spectrogram(sigrec[:, idx], nperseg=nperseg, nfft=sweepnfft, noverlap=int(
            0.5*nperseg), scaling="spectrum")
        ...
        p = ax.pcolormesh(taxis, faxis, 20*np.log10(spectest),
                        cmap="hot", vmin=-130)
        ax.set_yscale("log")
        ...
        p = ax.pcolormesh(taxis, faxis, 20*np.log10(specrec),
                        cmap="hot", vmin=-130)
        ax.set_yscale("log")
        ax.set_ylim((20, 20000))
        cb = fig.colorbar(p, ax=ax, orientation="horizontal", fraction=0.06)

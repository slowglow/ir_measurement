...
from numpy import log, exp, sin, pi
...
class stimulus:
    ...
    def generate(self, fs, duration, amplitude, repetitions, silenceAtStart, silenceAtEnd, sweeprange):
        ...
        lw = log(w2/w1)
        sinsweep = amplitude * \
            sin(w1*(numSamples-1)/lw * (exp(taxis*lw)-1))
        ...
        sinsweep = np.transpose(
            np.tile(np.transpose(sinsweep), repetitions))
